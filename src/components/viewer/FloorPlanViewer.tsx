...
import { FloorPlanMesh, computeSceneBounds } from './FloorPlanMesh'
import type { FloorPlanGeometry, FloorPlan3DOverrides } from './FloorPlanMesh'
...
export function FloorPlanViewer({
  ...
  // Compute scene bounds and dollhouse camera position
  const sceneBounds = computeSceneBounds(geometry)
  const [centerX, , centerZ] = sceneBounds.center
  const dollhouseHeight = Math.max(sceneBounds.size[0], sceneBounds.size[2]) * 0.8

  // Convert doorway hotspots to internal Hotspot3D format
  const converted3DHotspots: Hotspot3D[] = hotspots.map((h) => ({
    id: h.id,
    position: [h.position.x, h.position.y, h.position.z],
    label: h.tooltip ?? h.id,
  }))
  ...
  useEffect(() => {
    if (savedRoomData && savedRoomId) {
      const found = savedRoomData.find((r) => (r._id as string) === savedRoomId)
      if (found) {
  ...
  // Safe dollhouse camera height — fallback when bounds are too small
  const cameraHeight = Math.max(dollhouseHeight, 5)
  ...
  return (
    ...
    >
      ...
      <div style={{ flex: 1, position: 'relative', minWidth: 0, height: '100%' }}>
        ...
        <Canvas
          camera={{
            position: [centerX, cameraHeight, centerZ + cameraHeight],
            fov: 50,
          }}
...
// ---------------------------------------------------------------------------
// Simple 3D hotspot marker for doorway navigation
// ---------------------------------------------------------------------------

function HotspotMarker({ hotspot }: { hotspot: Hotspot3D }) {
  return (
